...
public class gameServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (session.getAttribute("informations_dee") != null && user!=null && gameSate!=null) {
            // casting
            ...
            if (informations_dee.containsKey(numerodee)) {
                ...
                // ajouter un message dans la requette
                req.setAttribute("message","vous avez jouer avec deux dées dans une meme partie");
                //on supprime les informations_dee dans la session pour que il peut jeux encore une fois
            ...
            if (informations_dee.size() < 3) {
                ...
                if (informations_dee.get(2) == 1 || informations_dee.get(2) == 6) {
                    user.setScore(0);
                    gameSate.setGameOver(true);
                    req.setAttribute("message","malheureusement vous avez ratté. les règles de jeux");
                    session.removeAttribute("informations_dee");
                    req.getRequestDispatcher(gameover).forward(req, resp);
                    return;
                }
...
//                gameContext.updateScore(user);
                ...
                gameSate.setGameOver(true);
                req.setAttribute("message","Game Over , jeux términée");
                session.removeAttribute("informations_dee");
                req.getRequestDispatcher(gameover).forward(req, resp);
                return;
            }
        ...
        // si auccun condition n'a été réussi donc c'est le cas d'une autre répition
        req.getRequestDispatcher(gamejsp).forward(req, resp);
        return;
    }
